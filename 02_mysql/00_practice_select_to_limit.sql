-- practice 유저 계정 및 db 생성
CREATE USER 'practice'@'%' IDENTIFIED BY  'practice';
SHOW databases;
USE mysql;
SELECT * FROM user;	
CREATE DATABASE practice_employdb;
GRANT ALL PRIVILEGES ON practice_employdb.* TO 'practice'@'%';	
...
SELECT EMP_ID, EMP_NAME from employee WHERE DEPT_CODE = 'D9';
...
SELECT EMP_NAME, SALARY from employee WHERE SALARY >= 3000000 AND DEPT_CODE = 'D6';
...
SELECT EMP_ID, EMP_NAME, SALARY from employee WHERE SALARY BETWEEN 3500000 AND 5500000;
...
SELECT EMP_NAME, SALARY from employee WHERE (JOB_CODE = 'J2' or JOB_CODE = 'J7') AND SALARY >= 2000000;
...
SELECT EMP_NAME from employee ORDER BY EMP_NAME ASC LIMIT 10;